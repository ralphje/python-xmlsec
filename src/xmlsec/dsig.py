...
class DSigError(Exception):
    """Thrown when something goes wrong inside the DSig class. This generally
    indicates some failure within the DSig module, but may also be caused by
...
class DSig(object):
    def __init__(self):
        """Creates a new DSig object. It will instantiate a local key
        manager.
        """
        ...
        if xmlsec.cryptoAppDefaultKeysMngrInit(self.key_manager) < 0:
            raise DSigError("Failed initializing the key manager")
    ...
    def load_key(self, path, password=None, name=None, cert_path=None, key_format='pem', cert_format='pem'):
        ...
        key_format = DSig._determine_key_format(key_format)
        cert_format = DSig._determine_key_format(cert_format)
        
        try:
            # Load key
            key = xmlsec.cryptoAppKeyLoad(path, key_format, password, None, None)
            if key is None:
                raise DSigError('Failed loading private key %s' % path)
            
            # Set key name
            if name and key.setName(name) < 0:
                raise DSigError('Failed setting key name of %s to %s' % (path, name))
            
            # Link certificate to key
            if cert_path and xmlsec.cryptoAppKeyCertLoad(key, cert_path, cert_format) < 0:
                raise DSigError('Failed loading certificate %s' % cert_path)
            
            # Load certificate into store
            if xmlsec.cryptoAppDefaultKeysMngrAdoptKey(self.key_manager, key) < 0:
                raise DSigError("Failed loading key %s into key manager." % path)
            
        except DSigError:
            raise
        except Exception as e:
            raise DSigError('Something went wrong loading key %s: %s' % (path, e))
    ...
    def load_cert(self, path, cert_format='pem', trust='trusted'):
        ...
        cert_format = DSig._determine_key_format(cert_format)
        trust = DSig._determine_trust_format(trust)
        
        try:
            if self.key_manager.certLoad(path, xmlsec.KeyDataFormatPem, trust) < 0:
                raise DSigError("Failed loading certificate %s into key manager" % path)
        except DSigError:
            raise
        except Exception as e:
            raise DSigError('Something went wrong loading certificate %s: %s' % (path, e))
    ...
    def sign(self, body, *args, **kwargs):
        ...
        try:
            # parse the xml
            (doc, node) = DSig._parse_dsig_xml(body)
            ...
            if dsig_ctx.sign(node) < 0:
                raise DSigError('Failed signing the xml.')
            ...
            raise DSigError('Something went wrong signing the xml: %s' % e)
    ...
    def template_sign(self, body, canonicalization, signing, references,
        ...
        canonicalization = DSig._determine_transform_format(canonicalization)
        signing = DSig._determine_transform_format(signing)
        ...
        try:
            ...
            if sign_node is None:
                raise DSigError('Failed creating a signature template.')
            
            doc.getRootElement().addChild(sign_node)
            
            # Create references
            for (reference, transforms) in references.items():
                ref = DSig._determine_transform_format(reference)
                
                ref_node = sign_node.addReference(ref, None, None, None)
                if ref_node is None:
                    raise DSigError('Failed adding reference %s to template.' % reference)
                
                # add transforms
                for transform in transforms:
                    trans = DSig._determine_transform_format(transform)
                    
                    if ref_node.addTransform(trans) is None:
                        raise DSigError('Failed adding transform %s to template.' % transform)
            
            # Add keyinfo
            if key_name or key_x509:
                keyinfo_node = sign_node.ensureKeyInfo(None)
                if keyinfo_node is None:
                    raise DSigError('Failed adding key info to template.')
                
                if key_name:
                    if keyinfo_node.addKeyName(key_name) is None:
                        raise DSigError('Failed adding key name to template.')
                    
                if key_x509:
                    if keyinfo_node.addX509Data() is None:
                        raise DSigError('Failed adding X509 data to template.')
    ...
    def verify(self, body):
        ...
        try:
            ...
            if dsig_ctx.verify(node) < 0:
                raise DSigError('Failed verifying signature.')
            ...
            raise DSigError('Something went wrong verifying the xml: %s' % e)
    ...
    def _determine_trust_format(truststring):
        ...
        if truststring == 'any':
            ...
            raise DSigError('Unknown trust format: %s' % truststring)
    ...
    def _determine_key_format(formatstring):
        ...
        if formatstring == 'binary':
            ...
            raise DSigError('Unknown key format: %s' % formatstring)
    ...
    def _parse_dsig_xml(body):
        ...
        if node is None:
            raise DSigError("Failed finding signature root node.")
    ...
    def _create_dsig_ctx(self):
        ...
        if dsig_ctx is None:
            raise DSigError('Failed creating signature context.')
        return dsig_ctx
    ...
    def _create_enc_ctx(self):
        ...
        if enc_ctx is None:
            raise DSigError('Failed creating encryption context.')
        return enc_ctx
